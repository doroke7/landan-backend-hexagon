...
const (
	MonitorService_WatchMetrics_FullMethodName = "/monitor.MonitorService/WatchMetrics"
)
...
type MonitorServiceClient interface {
	WatchMetrics(ctx context.Context, in *MonitorRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[MetricResponse], error)
}
...
func (c *monitorServiceClient) WatchMetrics(ctx context.Context, in *MonitorRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[MetricResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &MonitorService_ServiceDesc.Streams[0], MonitorService_WatchMetrics_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[MonitorRequest, MetricResponse]{ClientStream: stream}
...
type MonitorService_WatchMetricsClient = grpc.ServerStreamingClient[MetricResponse]
...
type MonitorServiceServer interface {
	WatchMetrics(*MonitorRequest, grpc.ServerStreamingServer[MetricResponse]) error
...
func (UnimplementedMonitorServiceServer) WatchMetrics(*MonitorRequest, grpc.ServerStreamingServer[MetricResponse]) error {
...
func _MonitorService_WatchMetrics_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(MonitorRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(MonitorServiceServer).WatchMetrics(m, &grpc.GenericServerStream[MonitorRequest, MetricResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type MonitorService_WatchMetricsServer = grpc.ServerStreamingServer[MetricResponse]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _MonitorService_WatchMetrics_Handler,
			ServerStreams: true,
		},
	},
